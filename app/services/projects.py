...
def _load_json_with_encoding_fallback(path: str) -> Dict[str, Any]:
    """Load JSON with a UTF-8 first policy and Windows-safe fallback.

    Some legacy project files may have been saved with CP1252/Latin-1.
    We try strict UTF-8 first, then fall back to CP1252 and Latin-1.
    """
    ...
    for encoding in encodings:
        try:
            ...
            # Decoded but still not valid JSON; propagate for caller handling.
            raise
    if last_decode_error is not None:
        raise last_decode_error
    raise ValueError(f"Cannot decode project file ({path})")
