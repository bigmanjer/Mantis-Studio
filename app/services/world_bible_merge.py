...
def _append_source_ref(entity: Entity, classified: Dict[str, Any]) -> None:
    source = (classified.get("source") or "").strip()
    excerpt = (classified.get("source_excerpt") or classified.get("excerpt") or "").strip()
    chapter_id = (classified.get("chapter_id") or "").strip()
    if not source and not excerpt:
        return
    refs = getattr(entity, "source_refs", None)
    if not isinstance(refs, list):
        refs = []
        entity.source_refs = refs
    key = (
        source.lower(),
        chapter_id.lower(),
        re.sub(r"\s+", " ", excerpt.lower())[:160],
    )
    existing = {
        (
            str(ref.get("source", "")).lower(),
            str(ref.get("chapter_id", "")).lower(),
            re.sub(r"\s+", " ", str(ref.get("excerpt", "")).lower())[:160],
        )
        for ref in refs
...
def apply_suggestion(project: Project, classified: Dict[str, Any]) -> Tuple[Optional[Entity], str]:
    ...
    if stype in ("update", "alias_only"):
        ...
        if not desc_changed and not aliases_changed:
            ...
            action = "alias_added"
        _append_source_ref(ent, classified)
        return ent, action
