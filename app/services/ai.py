...
def _normalize_provider(provider: str) -> str:
    return "openai" if str(provider or "").strip().lower() == "openai" else "groq"


def _get_streamlit_session_state() -> Optional[Any]:
...
def get_effective_key(provider: str, user_id: Optional[str] = None) -> tuple[str, str]:
    """Resolve an API key from session state, saved config, or server defaults."""
    del user_id
    provider = _normalize_provider(provider)
    session_state = _get_streamlit_session_state()
    if session_state is not None:
        session_key = (session_state.get("ai_session_keys") or {}).get(provider, "")
        if session_key:
            return str(session_key).strip(), "session"

    saved_key = (load_app_config().get(f"{provider}_api_key") or "").strip()
    if saved_key:
        return saved_key, "saved"

    if provider == "openai":
        default_key = (AppConfig.OPENAI_API_KEY or "").strip()
    else:
        default_key = (AppConfig.GROQ_API_KEY or "").strip()
    if default_key:
...
def test_groq_connection(base_url: str, api_key: str) -> bool:
    return bool(AIEngine(base_url=base_url).probe_models(api_key))
