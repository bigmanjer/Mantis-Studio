...
class Project:
    ...
    def add_chapter(self, title: str = "Untitled", content: str = "") -> Chapter:
        ...
        if (title == "Untitled" or title.startswith("Chapter")) and self.outline:
            ...
            if match:
                ...
                split_text = re.split(r" [-:] ", raw_text, maxsplit=1)
    ...
    def load(cls, path: str) -> "Project":
        ...
        for k, v in world_db_data.items():
            ...
            if "source_refs" in clean and not isinstance(clean["source_refs"], list):
                clean["source_refs"] = []
            try:
...
def _run_ui():
    ...
    from app.utils.auth import (
        ...
        update_user_profile as auth_update_user_profile,
    )
    ...
    def _handle_oauth_callback() -> None:
        provider_name = _query_value("oauth_provider")
        code = _query_value("code")
        state_value = _query_value("state")
        error_value = _query_value("error")
        if provider_name and provider_name != "google":
            return
        if not (code and state_value):
            return
        st.query_params.clear()
        st.session_state["show_auth_gate"] = True
        st.session_state["guest_mode"] = False
        if error_value:
            st.session_state["oauth_flash_error"] = f"Google sign-in cancelled or failed: {error_value}"
            st.rerun()
            return
        ok, msg, profile = complete_google_oauth(
            code=code,
            state=state_value,
            session_state=st.session_state,
        )
        if not ok:
            st.session_state["oauth_flash_error"] = msg
            st.rerun()
            return
        ...
        st.session_state.pop("oauth_google_nonce", None)
        st.session_state.pop("oauth_google_started_at", None)
        st.session_state["oauth_flash_success"] = "Signed in with Google."
        st.rerun()
    ...
    def _render_auth_gate() -> bool:
        ...
        st.html(
            ...
            .mantis-auth-signal:last-child {
                border-right: 0;
            }
            .mantis-auth-signal strong {
                display: block;
                font-size: 1.25rem;
                margin-bottom: 0.1rem;
            }
            .mantis-auth-signal span {
                color: var(--mantis-muted);
                font-size: 0.8rem;
                line-height: 1.35;
            }
            ...
            .mantis-auth-path:last-child {
                border-bottom: 0;
                padding-bottom: 0;
            }
            .mantis-auth-path strong {
                color: var(--mantis-text);
                font-size: 0.9rem;
            }
            .mantis-auth-path span {
                color: var(--mantis-muted);
                font-size: 0.86rem;
                line-height: 1.45;
            }
                        ...
                        if signup_submitted:
                            if signup_password != signup_confirm:
                                ...
                                if not ok or not user:
                                    ...
                                    st.session_state["new_recovery_code"] = user.get("recovery_code", "")
                                    apply_login_to_session(st.session_state, user, AppConfig.PROJECTS_DIR)
                                    st.session_state["guest_mode"] = False
                                    st.session_state["show_auth_gate"] = False
                                    st.session_state.project = None
    ...
    pending_recovery_code = st.session_state.pop("new_recovery_code", "")
    if pending_recovery_code:
        st.warning(
            f"Save this recovery code now: {pending_recovery_code}. It is only shown once and is required for forgot-password recovery."
        )
    ...
    def _score_model_for_mantis(model_name: str, provider: str, passed: bool) -> Dict[str, Any]:
        """Heuristic model guide for MANTIS workflows after connection tests."""
        ...
        if not passed:
            return {
                "score": 0,
                "tier": "Unavailable",
                "best_for": "Not recommended until the connection test passes.",
                "strengths": [],
                "cautions": ["Failed the live API test."],
            }

        if any(token in name for token in ("gpt-4.1", "gpt-4o", "o4", "o3", "opus")):
            score += 30
            strengths.extend(["chapter prose", "canon reasoning", "outline planning"])
        if any(token in name for token in ("70b", "90b", "120b", "405b", "large")):
            score += 20
            strengths.extend(["long-form drafting", "world consistency"])
        if any(token in name for token in ("llama-3.3", "llama3-70b", "llama-3.1-70b", "mixtral", "qwen")):
            score += 14
            strengths.extend(["fast outlining", "entity extraction"])
        if any(token in name for token in ("mini", "small", "8b", "7b", "3b", "instant")):
            score -= 8
            strengths.append("quick checks")
            cautions.append("May miss subtle canon or long chapter context.")
        if any(token in name for token in ("preview", "experimental", "beta")):
            score -= 6
            cautions.append("Preview models can change behavior.")
        if provider == "groq":
            score += 6
            strengths.append("fast iteration")
        if provider == "openai":
            score += 8
            strengths.append("polished prose and instruction following")
        if any(token in name for token in ("vision", "audio", "tts", "embedding", "whisper")):
            score -= 25
            cautions.append("Specialized model; not ideal for MANTIS writing tasks.")

        score = max(0, min(100, score))
        if score >= 78:
            tier = "Best fit"
            best_for = "Architect outlines, chapter drafting, coherence checks, and World Bible reasoning."
        elif score >= 58:
            tier = "Good fit"
            best_for = "Daily writing support, entity scans, and outline revisions."
        elif score >= 38:
            tier = "Usable"
            best_for = "Fast brainstorming, summaries, and simple scans."
        else:
            tier = "Limited"
            best_for = "Use only when better writing models are unavailable."

        dedup_strengths = list(dict.fromkeys(strengths))
        dedup_cautions = list(dict.fromkeys(cautions))
        return {
            "score": score,
            "tier": tier,
            "best_for": best_for,
            "strengths": dedup_strengths,
            "cautions": dedup_cautions,
        }

    def _render_model_fit_results(provider: str, tests: Dict[str, str]) -> None:
        rows = []
        for model_name, error_message in sorted(tests.items()):
            passed = not bool(error_message)
            fit = _score_model_for_mantis(model_name, provider, passed)
            rows.append(
                {
                    "Model": model_name,
                    ...
                    "MANTIS fit": fit["tier"],
                    "Score": fit["score"],
                    "Best for": fit["best_for"],
                    "Strengths": ", ".join(fit["strengths"]) or "-",
                    "Watch": error_message or ", ".join(fit["cautions"]) or "-",
                }
            )
        rows.sort(key=lambda row: (row["Status"] == "Works", row["Score"]), reverse=True)
        best = next((row for row in rows if row["Status"] == "Works"), None)
        if best:
            st.success(f"Recommended for MANTIS: {best['Model']} ({best['MANTIS fit']}, {best['Score']}/100)")
            if st.button(
                f"Use {best['Model']} for MANTIS",
                key=f"{provider}_use_best_model",
                use_container_width=True,
            ):
                if provider == "openai":
                    st.session_state.openai_model = best["Model"]
                    AppConfig.OPENAI_MODEL = best["Model"]
                else:
                    st.session_state.groq_model = best["Model"]
                    AppConfig.DEFAULT_MODEL = best["Model"]
                save_app_settings()
                st.toast(f"{best['Model']} selected.")
                st.rerun()
    ...
    def render_page_guide(title: str, purpose: str, steps: List[str], tips: Optional[List[str]] = None) -> None:
        """Small beginner guide for pages whose purpose is not obvious yet."""
        with st.expander(f"Learn this page: {title}", expanded=False):
    ...
    def _source_excerpt_for_entity(text: str, name: str, aliases: Optional[List[str]] = None, window: int = 110) -> str:
        raw = text or ""
        ...
        for candidate in candidates:
            ...
            if not clean:
                continue
            match = re.search(rf"\b{re.escape(clean)}\b", raw, flags=re.IGNORECASE)
            if match:
                start = max(0, match.start() - window)
                end = min(len(raw), match.end() + window)
                ...
                if end < len(raw):
                    excerpt = f"{excerpt}..."
                return excerpt
        return re.sub(r"\s+", " ", raw[:240]).strip()

    def _append_entity_source_ref(ent: Entity, source: str, excerpt: str = "", chapter_id: str = "") -> None:
        if ent is None:
            return
        refs = ent.source_refs if isinstance(getattr(ent, "source_refs", None), list) else []
        key = (
            (source or "").strip().lower(),
            (chapter_id or "").strip().lower(),
            re.sub(r"\s+", " ", (excerpt or "").strip().lower())[:160],
        )
        existing = {
            (
                str(ref.get("source", "")).strip().lower(),
                str(ref.get("chapter_id", "")).strip().lower(),
                re.sub(r"\s+", " ", str(ref.get("excerpt", "")).strip().lower())[:160],
            )
            for ref in refs
            if isinstance(ref, dict)
        }
        if key in existing or not (source or excerpt):
            ent.source_refs = refs
            return
        refs.append({"source": source or "Scan", "chapter_id": chapter_id, "excerpt": (excerpt or "")[:360]})
        ent.source_refs = refs[-12:]

    def extract_entities_ui(
        text: str,
        label: str,
        ...
        *,
        ...
        if (not bypass_cooldown) and last_scan and (time.time() - last_scan) < 15:
        ...
        for e in ents:
            ...
            source_excerpt = _source_excerpt_for_entity(
                raw_text,
                name,
                ...
                if st.session_state.openai_model_list:
                    with st.expander("Advanced: Test All Models", expanded=False):
                        ...
                        if st.session_state.openai_model_tests:
                            st.markdown("**MANTIS Model Guide:**")
                            st.caption("Scores favor models that should handle outlines, chapter prose, canon checks, and World Bible reasoning well.")
                            _render_model_fit_results("openai", st.session_state.openai_model_tests)
                ...
                if st.session_state.groq_model_list:
                    with st.expander("Advanced: Test All Models", expanded=False):
                        ...
                        if st.session_state.groq_model_tests:
                            st.markdown("**MANTIS Model Guide:**")
                            st.caption("Scores favor models that should handle outlines, chapter prose, canon checks, and World Bible reasoning well.")
                            _render_model_fit_results("groq", st.session_state.groq_model_tests)
    ...
    def render_user_settings():
        render_page_header(
            "User Settings",
            "Manage account identity, email, password recovery, and sign-in details.",
            tag="Account",
            key_prefix="user_header",
        )
        if st.session_state.get("guest_mode"):
            with st.container(border=True):
                st.markdown("### Guest session")
                st.caption("Create an account to save an identity, email, and password recovery details.")
                if st.button("Sign in / Create account", type="primary", use_container_width=True, key="user_guest_open_auth"):
                    _open_auth_from_app()
            return
        ...
        with st.container(border=True):
            ...
            st.caption("Your account ID is permanent. Username is flexible and can be shared by multiple users.")
            if is_super_admin:
                st.success("Built-in super admin: can manage users, roles, password resets, account status, and security access.")
            id_cols = st.columns(2)
            with id_cols[0]:
                st.text_input(
                    "Account ID",
                    value=str(st.session_state.get("account_id") or user_id or ""),
                    disabled=True,
                    key="user_settings_account_id",
                )
            with id_cols[1]:
                st.text_input(
                    "Internal UUID",
            ...
            with profile_cols[0]:
                profile_username = st.text_input(
                    ...
                    key="user_settings_username",
                )
            with profile_cols[1]:
                profile_display = st.text_input(
                    "Display name",
            ...
            with profile_cols[2]:
                profile_email = st.text_input(
                    ...
                    disabled=is_super_admin,
                    key="user_settings_email",
                )

            if st.button("Save user profile", type="primary", use_container_width=True, key="user_settings_save_profile"):
                ok, msg, user = auth_update_user_profile(
                    user_id=user_id,
        ...
        with st.container(border=True):
            ...
            current_pass = st.text_input(
                "Current password",
                type="password",
                key="user_settings_current_password",
            )
            new_pass = st.text_input(
                "New password",
                type="password",
                placeholder="Minimum 10 characters with uppercase, lowercase, and a number",
                key="user_settings_new_password",
            )
            confirm_pass = st.text_input(
                "Confirm new password",
                type="password",
                key="user_settings_confirm_password",
            )
            if st.button("Change password", use_container_width=True, key="user_settings_change_password"):
                if new_pass != confirm_pass:
                    st.error("Passwords do not match.")
                else:
                    ok, msg = auth_change_user_password(
                        user_id=user_id,
        ...
        with st.container(border=True):
            st.markdown("### Connected Sign-In")
            google_ready, _google_msg, _google_cfg = is_google_oauth_ready()
            st.caption("Connect external sign-in providers for this account.")
            provider_cols = st.columns(2)
            with provider_cols[0]:
                st.button(
                    "Google enabled" if google_ready else "Google not configured",
                    disabled=True,
                    use_container_width=True,
                    key="user_google_disabled",
                )
    ...
    def open_user_settings_from_sidebar():
        navigate_to_page("user")
    ...
    if not st.session_state.get("sidebar_collapsed", False):
        render_enhanced_sidebar(
            ...
            on_open_user_settings=open_user_settings_from_sidebar,
            on_navigate=navigate_to_page,
        )
    else:
        st.html(
            """
    ...
    def render_outline():
        ...
        with right:
            with st.container(border=True):
                ...
                st.caption("Design story structure, turning points, chapter purpose, and canon setup for the blueprint.")

                readiness_checks = [
                    ("Blueprint seed", bool(outline_text), "Add a premise, cast, or conflict seed before asking for a full structure."),
                    ("Chapter architecture", planned_chapter_count >= 3, "Ask Architect to create chapter markers and turning points."),
                    ("Story mass", outline_word_count >= 180, "Add stakes, setting, faction pressure, or character goals for stronger AI context."),
                    ("Canon hooks", bool(p.world_db), "Add or scan World Bible entries so the outline can reference existing canon."),
                ]
                ready_count = sum(1 for _label, ok, _fix in readiness_checks if ok)
                st.progress(ready_count / len(readiness_checks), text=f"Blueprint readiness {ready_count}/{len(readiness_checks)}")
                with st.expander("Blueprint readiness", expanded=True):
                    for label, ok, fix in readiness_checks:
                        status = "Ready" if ok else "Needs work"
                ...
                if st.button(
                    ...
                    full = ""
                    current_blueprint = (p.outline or "").strip()
                    world_context = "\n".join(
                        f"- {e.name} ({e.category}): {(e.description or '').strip()[:240]}"
                        for e in sorted(p.world_db.values(), key=lambda ent: (ent.category, ent.name.lower()))[:40]
                    )
                    prompt = (
                        "You are the story architect for MANTIS Studio. Build a clean, useful blueprint, not a vague summary.\n\n"
                        f"PROJECT: {p.title}\n"
                        f"GENRE: {p.genre or 'Unspecified'}\n"
                        f"TARGET CHAPTERS: {chaps}\n\n"
                        f"CURRENT BLUEPRINT:\n{current_blueprint[:3500] or '[No outline yet]'}\n\n"
                        f"WORLD BIBLE CONTEXT:\n{world_context or '[No canon entries yet]'}\n\n"
                        "Return a chapter-by-chapter architecture using this exact shape:\n"
                        "Chapter N: Title\n"
                        "- Purpose: what this chapter must accomplish\n"
                        "- Conflict: the pressure, obstacle, or reveal\n"
                        "- Character movement: how the protagonist or key cast changes\n"
                        "- Canon/world hooks: entities, locations, factions, or lore to reinforce\n"
                        "- End beat: the hook into the next chapter\n\n"
                        "Make the structure specific, sequential, and usable for drafting."
                    )
                    try:
    ...
    def render_world():
        ...
        render_page_guide(
            "World Bible",
            "World Bible is the story encyclopedia. It stores characters, places, factions, and lore so MANTIS can detect contradictions and keep AI writing grounded.",
            [
        ...
        mention_source_refs: Dict[str, List[Dict[str, str]]] = {}
        for ent in entries:
            ...
            for chap, lower_text in chapter_texts_lower:
                ...
                if hits:
                    ...
                    excerpt = _source_excerpt_for_entity(chap.content or "", ent.name, ent.aliases)
                    hit_sources.append(
                        {
                            "source": f"Chapter {chap.index}: {chap.title}",
                            "chapter_id": chap.id,
                            "excerpt": excerpt,
                        }
                    )
            ...
            mention_source_refs[ent.id] = hit_sources
        ...
        flagged_entity_ids = under_described_ids | collision_ids
        if has_chapter_text:
            flagged_entity_ids |= orphaned_ids
                ...
                if st.session_state.get(f"add_open_{category}", False):
                    with st.form(f"add_{category}"):
                        ...
                        if ok:
                            ...
                            if not name:
                                st.warning("Add a name before saving this World Bible entry.")
                                st.stop()
                            if category in {"Faction", "Lore"} and not description:
                                st.warning(f"Add a short description so this {category.lower()} is useful for canon checks.")
                                st.stop()
                            entity, action = p.upsert_entity(
                                name,
                                category,
                                description,
                                aliases=aliases,
                            ...
                            if entity is None:
                                st.warning("World Bible entry was not saved. Check the name and try again.")
                                st.stop()
                            if persist_project(p, action=f"world_add_{category.lower()}"):
                                st.session_state[f"add_open_{category}"] = False
                                st.session_state.pop(f"add_{cat_slug}_name", None)
                                st.session_state.pop(f"add_{cat_slug}_desc", None)
                                st.session_state.pop(f"add_{cat_slug}_aliases", None)
                                st.toast(
                                    f"{category} {'updated' if action == 'matched' else 'saved'}."
                                )
                                st.rerun()
                            st.error("Project save failed. Your entry is still in this session; try Save again.")
                        if cancel:
                            st.session_state[f"add_open_{category}"] = False
                            st.rerun()
                    ...
                    with st.container(border=True):
                        ...
                        with st.expander(detail_label, expanded=e.id == focus_entity):
                            ...
                            if refs:
                                ...
                                if has_chapter_text:
                                    st.caption("No chapter references found yet.")
                                else:
                                    st.caption("No chapter text to compare yet.")

                            stored_refs = list(getattr(e, "source_refs", []) or [])
                            live_refs = mention_source_refs.get(e.id, [])
                            combined_refs = []
                            ref_keys = set()
                            for ref in stored_refs + live_refs:
                                if not isinstance(ref, dict):
                                    continue
                                ref_key = (
                                    str(ref.get("source", "")).lower(),
                                    str(ref.get("chapter_id", "")).lower(),
                                    re.sub(r"\s+", " ", str(ref.get("excerpt", "")).lower())[:120],
                                )
                            ...
                            if combined_refs:
                                with st.expander("Source markers", expanded=False):
                                    for ref_idx, ref in enumerate(combined_refs[:8], start=1):
                                        st.markdown(f"**{ref_idx}. {ref.get('source') or 'Scan'}**")
                            ...
                            with d1:
                                if st.session_state.delete_entity_id == e.id:
                                    ...
                                    st.rerun()
                            with d2:
                                if st.button("Save", key=f"save_entity_{e.id}", type="primary", use_container_width=True):
                                        ...
                                        e.source_refs = [
                                            {
                                                ...
                                                "chapter_id": str(ref.get("chapter_id", "")),
                                                "excerpt": str(ref.get("excerpt", ""))[:360],
                                            }
                                            for ref in combined_refs[:12]
                                            if isinstance(ref, dict)
                                        ]
                                    if persist_project(p, action=f"world_save_{e.id}"):
                                        st.toast(f"{e.name} saved.")
                                        st.rerun()
    ...
    def render_memory():
        ...
        st.caption("Keep canon notes, timelines, style rules, and facts the AI should always know.")
        with st.container(border=True):
            ...
            with guide_cols[3]:
                ...
                st.caption("Voice, tense, formatting, and prose rules.")
        ...
        st.markdown("#### Style Guide")
        style_key = f"world_memory_style_{p.id}"
        style_val = st.text_area(
            "Style Guide",
            p.style_guide,
            height=180,
            key=style_key,
            help="Examples: third-person close, past tense, no modern slang, concise action paragraphs.",
        )
    ...
    def render_insights():
        ...
        render_page_guide(
            "Insights",
            "Insights is the project dashboard for risk and momentum. It does not write for you; it points out what probably needs attention next.",
            [
                "Check Risk Radar first to see contradictions, weak canon entries, or pending World Bible reviews.",
                "Use Next Best Action when you are unsure what to do next.",
                "Watch Momentum to see drafting progress and recent activity.",
                "Use AI Readiness to see whether your outline, memory, draft, and canon are strong enough for better AI results.",
                "Use Entity Coverage Focus to find characters or lore that need more detail or chapter usage.",
            ],
            [
                "Insights becomes more useful after you have at least one chapter and several World Bible entries.",
                "A flagged entity is not always wrong; it means MANTIS wants you to review it.",
            ],
        )
    ...
    def render_chapters():
        ...
        render_page_guide(
            "Editor",
            "This is where you write the actual manuscript chapter by chapter. MANTIS saves your draft, can summarize chapters, and can generate or revise prose when AI is connected.",
            [
                "Pick a chapter from the left list or use the chapter controls at the top.",
                "Write or paste text in the large editor. Autosave keeps the project file current.",
                "Use Save Chapter when you want MANTIS to also scan the chapter for World Bible entities.",
                "Use Update Summary after a chapter has content so Insights and future AI writing have context.",
                "Use the Assistant panel for AI drafting only after your outline and memory are ready.",
            ],
            [
                "The World Bible tracks people, places, factions, and lore.",
                "Memory tells the AI what must stay true or what style/context to remember.",
                "If canon risk is high, auto-writing is disabled until issues are reviewed.",
            ],
        )
    ...
    def _render_current_page() -> None:
        ...
        if st.session_state.page == "home":
            ...
            logger.debug("Rendering user settings page")
            with key_scope("user"):
