...
_DEFAULT_SECRETS = {
    _SUPABASE_URL_KEY: "https://ojccaojeazwkqeqrutzo.supabase.co",
    _SUPABASE_ANON_KEY: "sb_publishable_Iv3r0kQptOsNqcoNoFF8oA_Ty0nDBi4",
    _SUPABASE_REDIRECT_URL_KEY: "https://mantis-studio.streamlit.app/Account_Settings",
}
...
def _get_secret(name: str, default: str = "") -> str:
    ...
    try:
        ...
        value = default
    return str(_DEFAULT_SECRETS.get(name, value) or value)
...
def get_user_id_with_fallback(user: Any) -> str:
    """
    app/main.py calls this. Always return a stable user id string.
    """
    if not user:
...
# -----------------------------
# Optional: Login gate helper (if app/main.py uses it later)
# -----------------------------
def require_login(
...
def render_login_screen(intent: Optional[str] = None, allow_guest: bool = False) -> bool:
    ...
    if st.button("Open Account Access", use_container_width=True, key=ui_key("auth", "open_account")):
        try:
            ...
            st.info("Open the Account page from the sidebar navigation.")
