...
_ASSET_ALIASES = {
    "branding/mantis_wordmark.png": (
        "mantis_wordmark.png",
        "branding/mantis_lockup.png",
        "mantis_lockup.png",
        "NEW MANTIS BRANDING.png",
    ),
    "branding/mantis_lockup.png": (
        "mantis_lockup.png",
        "branding/mantis_wordmark.png",
    ...
    "branding/mantis_banner_dark.png": (
        "mantis_banner_dark.png",
        "branding/mantis_lockup.png",
        "mantis_lockup.png",
        "NEW MANTIS BRANDING.png",
    ),
...
def _source_image(assets_dir: Path) -> Optional[Path]:
    primary = assets_dir / "NEW MANTIS BRANDING.png"
    if primary.exists():
        return primary
    fallback = assets_dir / "branding" / "NEW MANTIS BRANDING.png"
    if fallback.exists():
        return fallback
    return None


def _resolve_direct_asset_path(
    assets_dir: Path,
    filename: str,
    _visited: Optional[Set[str]] = None,
) -> Optional[Path]:
    """Resolve direct assets with tolerant root/branding fallbacks."""
    ...
    if filename.startswith("branding/"):
        root_leaf = assets_dir / leaf
        if root_leaf.exists():
            return root_leaf
    else:
        branded = assets_dir / "branding" / leaf
        if branded.exists():
            return branded

    aliases = _ASSET_ALIASES.get(filename, ())
    for alias in aliases:
        alias_path = _resolve_direct_asset_path(assets_dir, alias, _visited)
...
def read_asset_bytes(assets_dir: Path, filename: str) -> Optional[bytes]:
    ...
    path = _resolve_direct_asset_path(assets_dir, filename)
    if path is None:
