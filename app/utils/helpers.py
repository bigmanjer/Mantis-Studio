...
def _attempt_connection_probe(ss) -> bool:
    """Attempt a one-time live provider connectivity probe when keys exist."""
    try:
        from app.services.ai import get_effective_key, test_groq_connection, test_openai_connection
    except Exception:
        return False

    user_id = ss.get("user_id")
    provider_checks = (
        ("groq", "groq_base_url", "groq_connection_tested", "groq_probe_attempted", test_groq_connection),
        ("openai", "openai_base_url", "openai_connection_tested", "openai_probe_attempted", test_openai_connection),
    )

    any_success = False
    for provider, base_url_key, tested_key, attempted_key, test_fn in provider_checks:
        if ss.get(tested_key) is True:
            any_success = True
            continue
        if ss.get(attempted_key):
            continue
        key_value, _ = get_effective_key(provider, user_id)
        if not key_value:
            continue
        base_url = ss.get(base_url_key, "")
        ok = bool(test_fn(base_url, key_value))
        ss[tested_key] = ok
        ss[attempted_key] = True
        if ok:
            any_success = True
...
def ai_connection_warning(st) -> None:
    ...
    tested = _has_tested_connection(ss)
    if not tested:
        tested = _attempt_connection_probe(ss)
