...
See app/README.md and docs/README.md for the current architecture notes.
...
def _run_ui():
    ...
    def render_home():
        ...
        try:
            banner_bytes = dashboard_banner_path.read_bytes() if dashboard_banner_path.exists() else None
        except Exception:
            banner_bytes = None
        ...
        with st.container(border=True):
            st.markdown("# DASHBOARD OVERVIEW ACTIVE")
            hero_cols = st.columns([2.4, 1], vertical_alignment="center")
            with hero_cols[0]:
                st.markdown("## Dashboard Overview")
                st.caption("Command your studio at a glance: focus, momentum, and launch points.")
                st.markdown(f"**Current focus:** {project_title}")
                st.caption(latest_chapter_label)
            with hero_cols[1]:
                if st.button(primary_label, type="primary", use_container_width=True, key="dashboard_primary_focus"):
                    if primary_target == "chapters" and latest_chapter_id:
                        st.session_state.curr_chap_id = latest_chapter_id
                    open_recent_project(primary_target)
        ...
        with main_cols[0]:
            with st.container(border=True):
                ...
                st.caption(latest_chapter_label)
                if st.button(primary_label, type="primary", use_container_width=True, key="dashboard_primary_next"):
                    if primary_target == "chapters" and latest_chapter_id:
                        st.session_state.curr_chap_id = latest_chapter_id
                    open_recent_project(primary_target)
        ...
        with main_cols[1]:
            ...
            with st.container(border=True):
                st.markdown("#### Utilities & support")
                st.caption("Admin shortcuts and documentation access.")
                s1, s2 = st.columns(2)
                with s1:
                    ...
                    if st.button(
                        ...
                        use_container_width=True,
                        help="Jump to AI Settings to configure Groq/OpenAI.",
                    ):
                        open_ai_settings()
                with s2:
    ...
    def render_outline():
        ...
        with st.container(border=True):
            ...
            with top1:
                new_title = st.text_input("Project Title", key="outline_project_title_input")
                ...
                for idx, e in enumerate(ents):
                    ...
                    with st.container(border=True):
                        ...
                        with st.expander(detail_label, expanded=e.id == focus_entity):
                            ...
                            st.caption(f"Aliases: {len(e.aliases or [])} | Created: {time.strftime('%Y-%m-%d', time.localtime(e.created_at))}")
        ...
        def create_next_chapter() -> None:
            ...
            if p.outline:
                ...
                if match:
                    ...
                    title = sanitize_chapter_title(re.split(r" [-:] ", raw, maxsplit=1)[0].strip()) or title
            p.add_chapter(title)
        ...
        with col_nav:
            with st.container(border=True):
                ...
                if st.button(
                    ...
                    if match:
                        raw = match.group(1).strip()
                        title = sanitize_chapter_title(re.split(r" [-:] ", raw, maxsplit=1)[0].strip())
                    else:
                        title = f"Chapter {next_idx}"
