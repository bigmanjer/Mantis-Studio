...
def render_enhanced_sidebar(version: str, project: Optional[Any], current_page: str, world_focus: str, debug_enabled: bool, key_scope: Optional[Callable], slugify: Callable, save_project_callback: Callable, close_project_callback: Callable) -> None:
    del debug_enabled
    from app.utils.navigation import get_nav_sections

    instance_id = uuid.uuid4().hex
    scope_fn = key_scope if callable(key_scope) else None

    def scoped(name: str, _scope_fn=scope_fn):
        if _scope_fn is not None:
            return _scope_fn(name)
        return nullcontext()
