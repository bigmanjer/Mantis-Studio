...
def cmd_qa(args: argparse.Namespace) -> int:
    audit_cmd = [sys.executable, "scripts/full_repo_audit.py"]
    audit = _run(audit_cmd, check=False, capture=True)
    if audit.stdout:
        print(audit.stdout)
    if audit.stderr:
        print(audit.stderr)
    ...
    if args.with_visual:
        visual_status = cmd_visual(args)
        return 1 if audit.returncode or visual_status else 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_test = sub.add_parser("test", help="Run pytest (default: tests)")
    p_test.add_argument("--target", default="tests")
