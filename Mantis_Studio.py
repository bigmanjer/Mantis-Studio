...
def _run_ui():
    ...
    def auth_is_configured(provider: str) -> bool:
        try:
            auth_config = st.secrets.get("auth", {})
            provider_config = auth_config.get(provider, {}) if isinstance(auth_config, dict) else {}
            if not isinstance(provider_config, dict):
                return False
            required = ("client_id", "client_secret", "server_metadata_url", "redirect_uri")
            if not all(provider_config.get(key) for key in required):
                return False
            cookie_secret = auth_config.get("cookie_secret") or provider_config.get("cookie_secret")
            return bool(cookie_secret)
        except Exception:
            return False

    def trigger_oidc_login(provider: str) -> None:
        if not hasattr(st, "login"):
            st.error("Streamlit login is unavailable. Upgrade Streamlit to enable OIDC.")
            return
        try:
            ...
            logger.warning("OIDC login failed for provider: %s", provider, exc_info=True)
            st.error("SSO login failed. Please contact your admin.")
    ...
    def render_auth():
        ...
        with hero_right:
            ...
            st.markdown("---")
            st.markdown("#### Continue with SSO")
            st.caption("Use your organization account if SSO is configured.")
            sso_cols = st.columns(2)
            with sso_cols[0]:
                if auth_is_configured("google"):
                    if st.button("Continue with Google", use_container_width=True, key="auth_oidc_google"):
                        trigger_oidc_login("google")
                else:
                    st.info("Google SSO is not configured. Ask an admin to add secrets.")
            with sso_cols[1]:
                if auth_is_configured("microsoft"):
                    if st.button("Continue with Microsoft", use_container_width=True, key="auth_oidc_microsoft"):
                        trigger_oidc_login("microsoft")
                else:
                    st.info("Microsoft SSO is not configured. Ask an admin to add secrets.")
